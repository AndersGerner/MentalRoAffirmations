import {
  ...
  TIER_PERSONAL_UNLIMITED_FEATURE_ID,
  tierFeatureConstants,
  tierPlanConstants,
} from '@/config/tierConstants'
import { tier } from '@/src/lib/tier'
import type { PricingTableData } from '@/src/types'
      ...
      if (
        ...
        const basePrice =
          ...
          Object.entries(tierPlan[1].features).find(
            (_feature) => _feature[0] === TIER_PERSONAL_UNLIMITED_FEATURE_ID
          )?.[1].base
        ...
        const tiers =
          ...
          tierPlan[1].features &&
          Object.entries(tierPlan[1].features).find(
            (_feature) => _feature[0] === TIER_ALL_INCLUSIVE_FEATURE_ID
          )?.[1].tiers
